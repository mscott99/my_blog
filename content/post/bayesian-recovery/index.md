...
$$P\_{z^{\ast}} = \sum\limits\_{i = 1}^{M}P_{z^{\ast}}( \cdot |c^{\ast} = i)P(c = i).$$
...
$$= 1 - TV(p\_{y}( \cdot |\hat{c} = 0),p\_{y}( \cdot |\hat{c} \neq 0))$$
...
$$\leq M\max\limits\_{i}(1 - TV(p\_{y}( \cdot |\hat{c} = i),p_{y}( \cdot |\hat{c} \neq i))).$$
...
\[$\{\phi\_{i}\}\_{i \in \lbrack n\rbrack}$]{.math .math-inline .is-loaded}
...
$$\alpha = \frac{1}{\sqrt{n}}\sup\limits\_{i \in \lbrack n\rbrack,x \in T \cap B\_{2}}\left\langle \phi\_{i}\sqrt{\frac{1}{p\_{i}}},x \right\rangle$$
...
\[$\alpha\_{i} = \sup\limits\_{x \in T \cap B\_{2}}\left\langle \phi\_{i},x \right\rangle$]{.math
...
$$\alpha = \max\limits\_{i \in \lbrack n\rbrack}\left( \frac{\alpha\_{i}}{\sqrt{np_{i}}} \right).$$
...
$$\alpha = \parallel \{\alpha\_{i}\} \parallel\_{2}^{2}.$$
